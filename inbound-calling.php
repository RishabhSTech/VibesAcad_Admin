			    	...
			    	<div class="row">
			    	    ...
			    	    <div  class="col-lg-6">
			    	       ...
			    	       <div  class="col-lg-6 text-end">
			    	        	<h6 class="mt-5 mb-3"> <button type="button" class="btn btn-primary MainColor "  data-bs-toggle="modal" data-bs-target="#staticBackdrop1">Add Inbound Call</button> </h6>
			    	    </div>
...
<script>
    ...
    jQuery(document).ready(function($) {
        ...
        $('#addInboundCallForm').submit(function(event) {
            
            event.preventDefault();
            
            $('#submit_inbound').text('Add Inbound Call');
            $('#submit_inbound').prop('disabled', false);
            
            var formData = $(this).serialize();
    
            $.ajax({
                type: 'POST',
                url: 'insert_inbound_call.php',
                data: formData,
                success: function(response) {
                    
                    loadTransactionData();
                    $('#submit_inbound').text('wait...');
                    $('#submit_inbound').prop('disabled', true);
                    
                    $('#staticBackdrop1').modal('hide');
    
                    $('#addInboundCallForm').trigger('reset');
